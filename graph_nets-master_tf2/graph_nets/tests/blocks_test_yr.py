...
class EdgeBlockTest(GraphModuleTest):
  ...
  def setUp(self):
    ...
    self._scale = 10.
    self._edge_model_fn = lambda features: features * self._scale
  ...
  def test_compatible_higher_rank_no_raise(self):
    ...
    input_graph = input_graph.map(lambda v: tf.transpose(a=v, perm=[0, 2, 1, 3]))
  ...
  def test_incompatible_higher_rank_inputs_raises(self,
    ...
    input_graph = input_graph.replace(
        **{field: tf.transpose(a=getattr(input_graph, field), perm=[0, 2, 1, 3])})
    network = blocks.EdgeBlock(
        functools.partial(snt.Conv2D, output_channels=10, kernel_shape=[3, 3]),
        use_edges=use_edges,
        use_receiver_nodes=use_receiver_nodes,
...
class NodeBlockTest(GraphModuleTest):
  ...
  def setUp(self):
    ...
    self._scale = 10.
    self._node_model_fn = lambda features: features * self._scale
  ...
  @parameterized.named_parameters(
      ("no received edges reducer", True, False, None, tf.math.unsorted_segment_sum),
      ("no sent edges reducer", False, True, tf.math.unsorted_segment_sum, None),
  )
...
class GlobalBlockTest(GraphModuleTest):
  ...
  def setUp(self):
    ...
    self._scale = 10.
    self._global_model_fn = lambda features: features * self._scale

  @parameterized.named_parameters(
      ("all_inputs, custom reductions",
       True, True, True, tf.math.unsorted_segment_sum, tf.math.unsorted_segment_mean),
  ...
  @parameterized.named_parameters(
      ("missing edges reducer", True, False, None, tf.math.unsorted_segment_sum),
      ("missing nodes reducer", False, True, tf.math.unsorted_segment_sum, None),
  )
  def test_missing_aggregation_raises_exception(
...
if __name__ == "__main__":
  ...
  #_scale = 10.
  #_edge_model_fn = lambda: lambda features: features * _scale

  _edge_model_fn=snt.Linear(10, name="mlp")
  ...
  tvars=edge_block.trainable_variables
  t=edge_block.trainable_weights
  ...
  #self.assertEqual(input_graph.nodes, output_graph.nodes)
  #self.assertEqual(input_graph.globals, output_graph.globals)
  erro=input_graph.nodes-output_graph.nodes
  erro1=input_graph.globals-output_graph.globals

  # with self.test_session() as sess:
  #     output_graph_out, model_inputs_out = sess.run(
  #         (output_graph, model_inputs))
  output_graph_out, model_inputs_out=output_graph, model_inputs

  expected_output_edges = model_inputs_out * _scale
  # self.assertNDArrayNear(
  #     expected_output_edges, output_graph_out.edges, err=1e-4)

  erro2=expected_output_edges-output_graph_out.edges
