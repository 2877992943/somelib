...
def _get_shape(tensor):
  ...
  if all(s is not None for s in shape_list):
    return shape_list
  shape_tensor = tf.shape(input=tensor)
...
def _axis_to_inside(tensor, axis):
  ...
  return tf.transpose(a=tensor, perm=perm)
...
def _build_placeholders_from_specs(dtypes,
  ...
  for field in ALL_FIELDS:
    ...
    if dtype is None or shape is None:
      ...
      dct[field] = keras.Input(shape=shape[1:],dtype=dtype,name=field)
...
def _compute_stacked_offsets(sizes, repeats):
  ...
  """
  sizes = tf.cast(tf.convert_to_tensor(value=sizes[:-1]), tf.int32)
...
def concat(input_graphs, axis, name="graph_concat"):
  ...
  with tf.compat.v1.name_scope(name):
    ...
    n_node_per_tuple = tf.stack(
        [tf.reduce_sum(input_tensor=gr.n_node) for gr in input_graphs])
    n_edge_per_tuple = tf.stack(
        [tf.reduce_sum(input_tensor=gr.n_edge) for gr in input_graphs])
...
def repeat(tensor, repeats, axis=0, name="repeat"):
  ...
  with tf.compat.v1.name_scope(name):
    ...
    indices = tf.reduce_sum(input_tensor=indicator_matrix, axis=1)
  ...
  for number_field, data_field in [[N_NODE, NODES], [N_EDGE, RECEIVERS]]:
    if dct.get(number_field) is None:
      if dct[data_field] is not None:
        dct[number_field] = tf.shape(input=dct[data_field])[0]
      else:
  ...
  for data_dict in data_dicts:
    ...
    for k, v in data_dict.items():
      if v is None:
        ...
        result[k] = tf.convert_to_tensor(value=v, dtype=dtype)
...
def _create_complete_edges_from_nodes_dynamic(n_node, exclude_self_edges):
  ...
  if exclude_self_edges:
    ...
    receivers = tf.boolean_mask(tensor=receivers, mask=ind)
    senders = tf.boolean_mask(tensor=senders, mask=ind)
    n_edge -= n_node
...
def fully_connect_graph_dynamic(graph,
  ...
  with tf.compat.v1.name_scope(name):
    ...
    _, senders_array, receivers_array, n_edge_array = tf.while_loop(
        cond=loop_condition, body=body, loop_vars=initial_loop_vars, back_prop=False)
...
def set_zero_node_features(graph,
  ...
  with tf.compat.v1.name_scope(name):
    n_nodes = tf.reduce_sum(input_tensor=graph.n_node)
...
def set_zero_edge_features(graph,
  ...
  with tf.compat.v1.name_scope(name):
    n_edges = tf.reduce_sum(input_tensor=graph.n_edge)
    return graph._replace(
...
def get_graph(input_graphs, index, name="get_graph"):
  ...
  with tf.compat.v1.name_scope(name):
    start_node_index = tf.reduce_sum(
        input_tensor=input_graphs.n_node[start_slice], name="start_node_index")
    start_edge_index = tf.reduce_sum(
        input_tensor=input_graphs.n_edge[start_slice], name="start_edge_index")
    end_node_index = start_node_index + tf.reduce_sum(
        input_tensor=input_graphs.n_node[graph_slice], name="end_node_index")
    end_edge_index = start_edge_index + tf.reduce_sum(
        input_tensor=input_graphs.n_edge[graph_slice], name="end_edge_index")
