...
class RelationNetwork(snt.Module):
...
def _received_edges_normalizer(graph,
  ...
  with tf.compat.v1.name_scope(name):
    return normalizer(
        ...
        num_segments=tf.reduce_sum(input_tensor=graph.n_node))
...
class SelfAttention(snt.Module):
  ...
  def __call__(self, node_values, node_keys, node_queries, attention_graph):
    ...
    attention_weights_logits = tf.reduce_sum(
        input_tensor=sender_keys * receiver_queries, axis=-1)
