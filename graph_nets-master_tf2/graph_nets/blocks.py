...
class EdgesToGlobalsAggregator(snt.Module):
...
class NodesToGlobalsAggregator(snt.Module):
...
class _EdgesToNodesAggregator(snt.Module):
  ...
  def __call__(self,graph):
    _validate_graph(graph, (EDGES, SENDERS, RECEIVERS,),
                    additional_message="when aggregating from edges.")
    num_nodes = tf.reduce_sum(input_tensor=graph.n_node)
